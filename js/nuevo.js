...
const crearProducto = (name, value, precioProducto) => {
  ...
  if (ARRAY_PRODUCTOS.some((x) => x.name == producto.name)) {
    Swal.fire({
      icon: "error",
      title: "Oops...",
      text: "Ya tiene ese producto en el changuito, no puede volver a agregarlo.",
    });
    return;
  }
...
function vaciarchango() {
  if (dataStorage !== null) {
    ...
    Swal.fire({
      icon: "error",
      title: "Oops...",
      text: "Se ha vaciado el changuito",
    });
  }
}
...
function verProductosEnChanguito() {
  ...
  if (dataStorage !== null) {
    ...
    for (let i = 0; i < dataStorage.length; i++) {
      ARRAY_PRODUCTOS.push(dataStorage[i]);
